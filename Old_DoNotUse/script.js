...
function handleButtonClick(buttonLabel) {
  ...
  switch (buttonLabel) {
    ...
    case "This page":
      startDialogue(
        "I made this page out of boredom, I wanted to make my own little silly website. It was also good training!",
      );
      break;
    case "Exit":
      startDialogue(
        "This is a website, if you wanna leave you can just close the tab.",
      );
      break;
    default:
...
function drawButtons(x, y, spacing, scale) {
  ...
  getButtons().forEach((btn, i) => {
    ...
    if (hoverIndex === i) {
      ctx.drawImage(
        assets.images.soulImg,
        btnX - 13.9 * scale,
        btnY - fontSize + 6.9 * scale,
        7.5 * scale,
        7.5 * scale,
      );
    }
